...
if AutoRepair == 0
    ...
    for j=1:num_sess
        ...
        if strcmp(spm_ver,'spm5')
            ...
            nscan=SPM.nscan(j);
            Pnew{j}   = SPM.xY.P(cnt:(cnt+nscan-1),:);
            cnt=cnt+nscan;
        else
...
Deweight = 1;
nsess = num_sess;
%nscans = session_size; %commented out by Jin
if Deweight == 1
    ...
    try
        index = 1;
        for isess = 1:nsess
            nscans = SPM.nscan(i);  %This is modified by Jin Wang as we do not have equal number of trials in each session
            %index = (isess-1)*nscans + 1;  % first image of session isess.
            imagedir = fileparts(scans(index,:));
            %deweightlist = fullfile(imagedir,'art_deweighted.txt'); %We do
            %not want to deweight too much data and we applied movement
            %regressors in the model to remove small movement effects.
            deweightlist = fullfile(imagedir,'art_repaired.txt');
